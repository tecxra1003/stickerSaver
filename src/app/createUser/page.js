...
export default function Create() {
    ...
    const createUser = async (e) => {
        ...
        try {
            const createdUser = await fetch('/api/user/create', {
                method: 'POST',
                headers: {
                        ...
                        <div className="mt-5 sm:mx-auto sm:w-full sm:max-w-sm">
                            <form className="space-y-6" action="#" method="POST">
                                ...
                                <div>
                                    ...
                                    <div className="mt-2">
                                        <input value={user.password} onChange={(e) => { setUser(user => ({ ...user, password: e.target.value })) }} type="password" autoComplete="new-password" required className="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" />
                                    </div>
                                </div>
                                <div>
                                    <div className="flex items-center justify-between">
